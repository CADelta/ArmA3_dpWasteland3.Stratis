...
BTC_revive_time_max = 600;		//Org = 600
BTC_who_can_revive  = ["Man"];
//BTC_loop_check    = 0;
BTC_disable_respawn = 0;		// 1; Completly removes the respawn button.
BTC_respawn_gear    = 1;
BTC_active_lifes    = 0;
BTC_lifes           = 10;
...
BTC_camera_unc      = 0;									//DO NOT CHANGE IN BTC Revive Lite version!
BTC_camera_unc_type = ["Behind the back","High","Free"];	//DO NOT CHANGE IN BTC Revive Lite version!
BTC_respawn_time    = 0;		//Black screen and countdown timer after you respawn.
...
BTC_pvp = 0; 					//If "0" everyone can revive. If "1" Enemies can not revive.
...
//CALL COMPILE FNCs
call compile preprocessFile "addons\=BTC=_revive_lite\=BTC=_revive_lite_functions.sqf";
...
[] spawn
{
	...
	{
		player addEventHandler ["HandleDamage", BTC_fnc_handledamage_gear];
	};
	...
	if (BTC_active_mobile == 1) then
	{
		switch (true) do
		{
			case (BTC_side == west) : {{private ["_veh"];_veh = _x;_spawn = [_x] spawn BTC_mobile_marker;{_x addAction [("<t color=""#ED2744"">") + ("Move to mobile " + _veh) + "</t>","addons\=BTC=_revive_lite\=BTC=_revive_lite_addAction.sqf",[[_veh],BTC_move_to_mobile], 8, true, true, "", format ["[""%1""] call BTC_mobile_check",_veh]];} forEach BTC_objects_actions_west;} forEach BTC_vehs_mobile_west_str;};
			case (BTC_side == east) : {{private ["_veh"];_veh = _x;_spawn = [_x] spawn BTC_mobile_marker;{_x addAction [("<t color=""#ED2744"">") + ("Move to mobile " + _veh) + "</t>","addons\=BTC=_revive_lite\=BTC=_revive_lite_addAction.sqf",[[_veh],BTC_move_to_mobile], 8, true, true, "", format ["[""%1""] call BTC_mobile_check",_veh]];} forEach BTC_objects_actions_east;} forEach BTC_vehs_mobile_east_str;};
			case (str (BTC_side) == "guer") : {{private ["_veh"];_veh = _x;_spawn = [_x] spawn BTC_mobile_marker;{_x addAction [("<t color=""#ED2744"">") + ("Move to mobile " + _veh) + "</t>","addons\=BTC=_revive_lite\=BTC=_revive_lite_addAction.sqf",[[_veh],BTC_move_to_mobile], 8, true, true, "", format ["[""%1""] call BTC_mobile_check",_veh]];} forEach BTC_objects_actions_guer;} forEach BTC_vehs_mobile_guer_str;};
			case (BTC_side == civilian) : {{private ["_veh"];_veh = _x;_spawn = [_x] spawn BTC_mobile_marker;{_x addAction [("<t color=""#ED2744"">") + ("Move to mobile " + _veh) + "</t>","addons\=BTC=_revive_lite\=BTC=_revive_lite_addAction.sqf",[[_veh],BTC_move_to_mobile], 8, true, true, "", format ["[""%1""] call BTC_mobile_check",_veh]];} forEach BTC_objects_actions_civ;} forEach BTC_vehs_mobile_civ_str;};
		};
	}
	else
	{
