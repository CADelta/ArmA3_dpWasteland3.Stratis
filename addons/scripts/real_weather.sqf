	...
	*/
	...
	// Mission starting date is 01/06/2035 at 06:00
	_startingdate = [2035, 9, 10, 6, 0];
	...
	// we check the skiptime for 10 seconds	(Not used in Real-time mode)
	...
	while {true} do
	{
		...
		if ((date select 3 > 2) && (date select 3 < 6)) then		// date select 3 = hours
		{
			_fog = 0.4 + (random 0.6);
		}
		else
