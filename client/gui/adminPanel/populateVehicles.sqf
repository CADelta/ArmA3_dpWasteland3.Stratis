...
private ["_allVehicles","_switch","_vehicleType","_vehicleSummary","_dialog","_vehicleListBox","_weaponText","_userText","_damageText","_speedText"];
...
if ((_uid in dpModerators) || (_uid in serverCADministrators)) then
{
	...
	switch (_switch) do
	{
		...
		{
	    	{
			    ...
			    if (_vehicleType isKindOf "Car") then
				{
					...
					{
	                	_vehicleSummary = format["[Class: Other] [Type: %1]",_vehicleType];		// Class: Car
				        _index = _vehicleListBox lbAdd format["%1",_vehicleSummary];
				        _vehicleListBox lbSetData [_index, str (_x)];
	                };
			    };
		...
		case 4:
		{
		  	{
			   	_vehicleType = Format["%1", typeOf _x];
			    if (((_vehicleType isKindOf "MotorCycle") || (_vehicleType isKindOf  "Car") || (_vehicleType isKindOf "Tank")) && (format ["%1", _x getVariable "authorisedVehicle"] != "1")) then
				{
	            	if (_vehicleType isKindOf "MotorCycle") then
					{
	                	_vehicleSummary = format["[Class: MotorCycle] [Type: %1]",_vehicleType];
	                ...
	                if (!(_vehicleType isKindOf "MotorCycle") && !(_vehicleType isKindOf "Truck") && !(_vehicleType isKindOf "Wheeled_APC") && !(_vehicleType isKindOf "Tank") && !(_vehicleType isKindOf "Helicopter") && !(_vehicleType isKindOf "Plane")) then
					{
	                	_vehicleSummary = format["[Class: Other] [Type: %1]",_vehicleType];
				        _index = _vehicleListBox lbAdd format["%1",_vehicleSummary];
				        _vehicleListBox lbSetData [_index, str (_x)];
	                };
			    };
