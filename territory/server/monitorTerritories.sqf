...
_onCaptureStarted =
{
    ...
    if (_ownerSideStr != "") then
    {
        ...
        [[_msg], "territoryActivityHandler", _sideObject, false] call BIS_fnc_MP;
    };
};
...
_onCaptureFinished =
{
    ...
    [[_msg, _captureValue], "territoryActivityHandler", _sideObject, false] call BIS_fnc_MP;

    _msgOthers = format ["%1 has captured %2", _descriptiveSideName, _captureDescription];
    [[_msgOthers], "territoryActivityHandler", _otherSideObjects, false] call BIS_fnc_MP;
};
...
while {true} do
{
    ...
    //diag_log format["TERRITORY SYSTEM: realLoopTime was %1", realLoopTime];       // ***CAD*** Disabled this, because I don't want the rpt file to get super spammed.
};
